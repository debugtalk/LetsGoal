...
实现 M2.6 阶段 1：入口（Skill 触发 + 飞书文档需求流）。
...
- 新增类型字段使用 snake_case
- LoopConfig 新增字段均为可选（不破坏现有配置）
- 飞书文档操作失败时给出明确错误提示，不静默跳过
- ReviewOutput.confidence 范围 [0, 1]
- 现有 137 个测试全部通过后才能提交

## 禁止改动

- core/references/loop-protocol.md
- directions/development/DIRECTION.md（已更新）
- docs/roadmap.md（已更新）
- docs/design.md（已更新）
